...
class JobService:
    ...
    async def init_job(self, job: InitJob) -> None:
        ...
        await self.job_repo.create_job(job=create)

    async def update_progress(
        self, *, job_id: UUID, percent: int, step: str | None = None
    ) -> None:
        ...
        current = await self.job_repo.get_status(job_id=job_id)
        ...
        await self.job_repo.update_progress(
            job_id=job_id, percent=new_percent, step=norm_step
        )
    ...
    async def update_status(
        ...
        *,
        ...
        cur = await self.job_repo.get_status(job_id=job_id)
        cur_status = (
    ...
    async def fail_job(
        self, *, job_id: UUID, exc: Exception, last_step: str | None = None
    ) -> None:
    ...
    async def get_status(self, job_id: UUID) -> JobStatusResponse:
        data = await self.job_repo.get_status(job_id=job_id)
        if data is None:
    ...
    async def _apply_corrections_to_embeddings(self, *, job_id, proposed_obj) -> bool:
        refs = await self.job_repo.get_job_document_refs(job_id=job_id)
        if refs:
    ...
    async def review_job(
        ...
        status = await self.get_status(job_id=job_id)
        ...
        if new_status is JobStatus.COMPLETED and proposed_obj is not None:
            await self._apply_corrections_to_embeddings(
                job_id=job_id, proposed_obj=proposed_obj
            )
