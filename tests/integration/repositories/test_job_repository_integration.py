...
async def test_can_create_job(session, job_created):
    result = await job_repository.get(session, job_id=job_created.id)
...
async def test_can_create_another_job_for_document(session, job_created):
    ...
    job = await job_repository.create(session, job=new_job)
...
async def test_can_delete_job(session, job_created):
    deleted = await job_repository.delete(session, job_id=job_created.id)
    assert deleted is True

    with pytest.raises(RecordNotFoundError):
        await job_repository.get(session, job_id=job_created.id)

    # Second delete is a no-op
    second = await job_repository.delete(session, job_id=job_created.id)
...
async def test_can_update_job(session, job_created):
    ...
    job.percent = 100
    job.step = 'review'
    updated_job = await job_repository.update(session, job=job)
...
async def test_get_for_fingerprint(session, ingestion_created):
    ...
    job = await job_repository.get_for_fingerprint(session, document_id=document_id, fingerprint=fingerprint)
    assert job is not None

    job2 = await job_repository.get_for_fingerprint(session, document_id=document_id, fingerprint='foo')
    assert job2 is None
