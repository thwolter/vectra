...
from app.repositories import embeddings_repository, job_repository
...
async def job_uploaded(session, apple_report_first_page_upload, upload_service):
    ...
    yield init, file
    await job_repository.delete(session, job_id=init.job_id)
...
async def test_init_upload_end_to_end_uses_database(session, job_uploaded):
    ...
    exists = await embeddings_repository.exists(session, digest=digest, collection=CollectionEnum.DEFAULT.value)
