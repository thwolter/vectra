...
def _ctype_of(f):
    return (
        f['content_type'] if isinstance(f, dict) else getattr(f, 'content_type', None)
    )
...
async def test_save_original_and_delete_success(
    ...
    assert any(
        (_key_of(f) or '').endswith('.pdf') or (_key_of(f) or '').endswith('.pdf.gz')
        for f in files
    )
...
async def test_save_markdown_then_load_and_delete_success(base_prefix, digest_str):
    ...
    ok = await store.delete(
        document_id=document_id, delete_markdown=True, delete_original=False
    )
    assert ok is True
...
async def test_info_with_both_files_success(
    base_prefix, apple_report_first_page_upload
):
    store = LocalFileStore(CollectionEnum.DEFAULT, base_path=base_prefix)
    file = TemporaryUploadFile.from_upload(apple_report_first_page_upload)
