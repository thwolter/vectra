...
async def ingestion_created(session, job_created, digest_random):
    ...
    record = await IngestionRepository.create(session, data=ingestion_create)
    await JobRepository.update(session, job=JobUpdate(id=job_created.id, status=JobStatus.NEEDS_REVIEW))
...
async def ingestion_created_with_completed(session, job_created, digest_random):
    ...
    record = await IngestionRepository.create(session, data=ingestion_create)
    await JobRepository.update(session, job=JobUpdate(id=job_created.id, status=JobStatus.COMPLETED))
    await JobService().ingestion_created(session, job_id=job_created.id, ingestion=record)
    yield record
    await session.delete(record)
