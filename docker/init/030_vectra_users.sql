...
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = vectra_user) THEN
        EXECUTE format('CREATE ROLE %I LOGIN PASSWORD %L', vectra_user, app_password);
    ELSE
        EXECUTE format('ALTER ROLE %I WITH LOGIN PASSWORD %L', vectra_user, app_password);
    END IF;
    ...
    EXECUTE format('GRANT vectra_rw TO %I', vectra_user);
